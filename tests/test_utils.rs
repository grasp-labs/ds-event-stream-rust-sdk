...
fn test_get_topic_function_exists() {
    ...
    let result = std::panic::catch_unwind(|| get_topic("localhost:9092", "username", "password", "test-topic"));
...
fn test_list_topics_function_exists() {
    // Test that the function exists and can be called
    // This will fail in test environment without Kafka, which is expected
    let result = std::panic::catch_unwind(|| list_topics("localhost:9092", "username", "password"));
